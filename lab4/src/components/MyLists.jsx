        ...
        <div className='px-16 py-8 m-8 bg-[#242323]'>
            ...
            <div className='grid grid-cols-3'>
                <div id='col1' className=' m-4 p-2 grid grid-rows-2 justify-center rounded-lg bg-[#0e7f2c]'>
                    <ul>
                        <li className='text-white justify-center flex items-center'>My Lists</li>
                        {lists.map((list, index) => (
                            <li className='p-2 flex items-center justify-center' key={index}>{list} <IoIosArrowDroprightCircle size={20} className='icons' onClick={handleInfo}/></li>
                        ))}
                    </ul>
                </div>
